...
final class SqliteSchemaParserTest extends \PHPUnit\Framework\TestCase
{
    public function testValidCreateTableReturnsNonNull(): void
    {
        $parser = new SqliteSchemaParser();
        $result = $parser->parse('CREATE TABLE users (id INTEGER PRIMARY KEY NOT NULL, name TEXT NOT NULL, email TEXT, UNIQUE (email))');
    ...
    {
        $parser = new SqliteSchemaParser();
        $definition = $parser->parse('CREATE TABLE users (id INTEGER PRIMARY KEY NOT NULL, name TEXT NOT NULL, email TEXT, UNIQUE (email))');
    ...
    public function testParsedNotNullColumnsMatchExpected(): void
    {
        ...
        $definition = $parser->parse('CREATE TABLE users (id INTEGER PRIMARY KEY NOT NULL, name TEXT NOT NULL, email TEXT, UNIQUE (email))');
        self::assertNotNull($definition);
        self::assertSame([], array_diff(['id', 'name'], $definition->notNullColumns));
    }
    ...
    {
        $parser = new SqliteSchemaParser();
        $result = $parser->parse('CREATE VIRTUAL TABLE IF NOT EXISTS main.fts_articles USING fts5(' . 'title, "article body" UNINDEXED, ' . "tokenize='porter unicode61', prefix='2 3')");
        self::assertNotNull($result);
    ...
    {
        $result = (new SqliteSchemaParser())->parse("CREATE VIRTUAL TABLE articles USING fts5(tokenize='porter', content='source', " . 'title, body UNINDEXED);');
        self::assertNotNull($result);
    ...
    {
        $result = (new SqliteSchemaParser())->parse('CREATE VIRTUAL TABLE articles USING fts5(title) ;');
        self::assertNotNull($result);
    ...
    public function testParseWithDefaultValues(): void
    {
        ...
        self::assertSame(['status' => "'active'", 'created_at' => 'CURRENT_TIMESTAMP'], $result->columnDefaults);
    }

    public function testParsePreservesParenthesizedDefaultExpressionBeforeConstraint(): void
    {
        $result = (new SqliteSchemaParser())->parse("CREATE TABLE t (label TEXT DEFAULT (printf('%s,%s', 'a', 'b')) NOT NULL, enabled INTEGER DEFAULT 1)");
        self::assertNotNull($result);
        self::assertSame(['label' => "(printf('%s,%s', 'a', 'b'))", 'enabled' => '1'], $result->columnDefaults);
    }
    ...
    {
        $definition = (new SqliteSchemaParser())->parse('CREATE TABLE users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT)');
        self::assertNotNull($definition);
    ...
    {
        $definition = (new SqliteSchemaParser())->parse('CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT) WITHOUT ROWID');
        self::assertNotNull($definition);
    ...
    {
        $definition = (new SqliteSchemaParser())->parse('CREATE TABLE measurements (id INTEGER PRIMARY KEY, sensor TEXT NOT NULL, reading REAL NOT NULL) STRICT');
        self::assertNotNull($definition);
        self::assertSame(['id', 'sensor', 'reading'], $definition->columns);
        self::assertSame(['id' => 'INTEGER', 'sensor' => 'TEXT', 'reading' => 'REAL'], $definition->columnTypes);
        self::assertSame(['id' => IdentityGenerationStrategy::MaxValue], $definition->identityStrategies);
    }

    public function testStrictAndWithoutRowidOptionsAreAcceptedInEitherOrder(): void
    {
        $strictLast = (new SqliteSchemaParser())->parse('CREATE TABLE events (id INTEGER PRIMARY KEY, payload BLOB) WITHOUT ROWID, STRICT;');
        $withoutLast = (new SqliteSchemaParser())->parse('CREATE TABLE events (id INTEGER PRIMARY KEY, payload BLOB) STRICT, WITHOUT ROWID');
    ...
    {
        $definition = (new SqliteSchemaParser())->parse("CREATE TEMP TABLE events (id INTEGER PRIMARY KEY, payload BLOB) /* option */ STRICT\n");
        self::assertNotNull($definition);
    ...
    {
        self::assertNull((new SqliteSchemaParser())->parse('CREATE TABLE events (id INTEGER PRIMARY KEY) ' . $suffix));
    }
    /**
    ...
    {
        $definition = (new SqliteSchemaParser())->parse("CREATE TABLE users (id INTEGER PRIMARY KEY, note TEXT DEFAULT 'WITHOUT ROWID')");
        self::assertNotNull($definition);
        self::assertSame(['id' => IdentityGenerationStrategy::MaxValue], $definition->identityStrategies);
    }
    ...
    {
        $definition = (new SqliteSchemaParser())->parse('CREATE TABLE users (id INTEGER PRIMARY KEY, value TEXT CHECK (WITHOUT ROWID))');
        self::assertNotNull($definition);
        self::assertSame(['id' => IdentityGenerationStrategy::MaxValue], $definition->identityStrategies);
    }
    ...
    {
        $definition = (new SqliteSchemaParser())->parse('CREATE TABLE users (id INTEGER PRIMARY KEY, value TEXT CHECK (WITHOUT ROWID)) WITHOUT ROWID');
        self::assertNotNull($definition);
        self::assertSame([], $definition->identityStrategies);
    }
    ...
    {
        $definition = (new SqliteSchemaParser())->parse('CREATE TABLE users (id INTEGER PRIMARY KEY, PRIMARY KEY (id))');
        self::assertNotNull($definition);
    ...
    {
        $result = (new SqliteSchemaParser())->parse('CREATE TABLE t (a INTEGER, stored_value INTEGER GENERATED ALWAYS AS (a * 2) STORED, ' . 'virtual_value INTEGER AS (a * 3) VIRTUAL)');
        self::assertNotNull($result);
        self::assertSame(['stored_value' => '(a * 2)', 'virtual_value' => '(a * 3)'], $result->generatedExpressions);
    }
    ...
    public function testConstraintKeywordPrefixesRemainColumnNames(): void
    {
        ...
        $result = $parser->parse($sql);
        self::assertNotNull($result);
        self::assertSame(['id', 'check_in', 'checkout_date', 'unique_value', 'constraint_name', 'foreign_key_id'], $result->columns);
    }
}
