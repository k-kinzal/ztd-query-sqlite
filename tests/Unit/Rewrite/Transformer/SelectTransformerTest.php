...
final class SelectTransformerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testCteInjectedSqlStartsWithWith(): void
    {
        $transformer = new SelectTransformer();
        $sql = 'SELECT * FROM users WHERE id = 1';
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice', 'email' => 'alice@example.com']], 'columns' => ['id', 'name', 'email'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER'), 'name' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'TEXT'), 'email' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'TEXT')]]];
    ...
    public function testCteContainsSelectUnionStructure(): void
    {
        $transformer = new SelectTransformer();
        $sql = 'SELECT * FROM users WHERE id = 1';
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice', 'email' => 'alice@example.com'], ['id' => 2, 'name' => 'Bob', 'email' => 'bob@example.com']], 'columns' => ['id', 'name', 'email'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER'), 'name' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'TEXT'), 'email' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'TEXT')]]];
        $result = $transformer->transform($sql, $tables);
        $upper = strtoupper($result);
        self::assertTrue(str_contains($upper, 'UNION ALL') || str_contains($upper, 'VALUES'), 'Multiple data rows should produce UNION ALL or VALUES in the CTE, got: ' . $result);
    ...
    public function testEmptyRowsWithColumnsReturnsWithClause(): void
    {
        $transformer = new SelectTransformer();
        $sql = 'SELECT * FROM users WHERE id = 1';
        $tables = ['users' => ['rows' => [], 'columns' => ['id', 'name', 'email'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER'), 'name' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'TEXT'), 'email' => new ColumnDeclaration(ColumnTypeFamily::STRING, 'TEXT')]]];
        $result = $transformer->transform($sql, $tables);
        self::assertStringStartsWith('WITH', ltrim($result));
        self::assertNotEmpty($result);
    }
    ...
    {
        $result = (new SelectTransformer())->transform('SELECT total FROM orders', ['orders' => ['rows' => [['qty' => 5, 'unit_price' => 10, 'total' => null]], 'columns' => ['qty', 'unit_price', 'total'], 'columnTypes' => [], 'generatedExpressions' => ['total' => '(qty * unit_price)']]]);
    ...
    {
        $tables = ['users' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []], 'active_users' => ['viewSql' => 'SELECT * FROM users WHERE id > 0'], 'active_user_count' => ['viewSql' => 'SELECT count(*) AS total FROM active_users']];
        self::assertSame("WITH \"users\" AS (SELECT CAST(1 AS INTEGER) AS \"id\"),\n\"active_users\" AS (SELECT * FROM users WHERE id > 0),\n\"active_user_count\" AS (SELECT count(*) AS total FROM active_users)\nSELECT * FROM active_user_count", (new SelectTransformer())->transform('SELECT * FROM active_user_count', $tables));
    }
    ...
    public function testUsesInjectedValueRenderer(): void
    {
        ...
        $tables = ['users' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []]];
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['products' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []]];
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [], 'columns' => ['id', 'name'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER'), 'name' => new ColumnDeclaration(ColumnTypeFamily::TEXT, 'TEXT')]]];
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice'], ['id' => 2, 'name' => 'Bob']], 'columns' => ['id', 'name'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER'), 'name' => new ColumnDeclaration(ColumnTypeFamily::TEXT, 'TEXT')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('WITH', $result);
        self::assertStringContainsString('"users"', $result);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice']], 'columns' => ['id', 'name'], 'columnTypes' => []], 'orders' => ['rows' => [['id' => 1, 'total' => 100]], 'columns' => ['id', 'total'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('"users"', $result);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['items' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []], 'users' => ['rows' => [['id' => 2]], 'columns' => ['id'], 'columnTypes' => []]];
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice']], 'columns' => ['id', 'name'], 'columnTypes' => []]];
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => null]], 'columns' => ['id', 'name'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER'), 'name' => new ColumnDeclaration(ColumnTypeFamily::TEXT, 'TEXT')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [], 'columns' => ['id'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice']], 'columns' => ['id', 'name'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER'), 'name' => new ColumnDeclaration(ColumnTypeFamily::TEXT, 'TEXT')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 42]], 'columns' => ['id'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['name' => 'hello']], 'columns' => ['name'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['active' => true]], 'columns' => ['active'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['active' => false]], 'columns' => ['active'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['score' => 3.14]], 'columns' => ['score'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['name' => "it's"]], 'columns' => ['name'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 42]], 'columns' => ['id'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString("CAST('42' AS INTEGER) AS \"id\"", $result);
    }
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['active' => true]], 'columns' => ['active'], 'columnTypes' => ['active' => new ColumnDeclaration(ColumnTypeFamily::BOOLEAN, 'BOOLEAN')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['score' => 3.14]], 'columns' => ['score'], 'columnTypes' => ['score' => new ColumnDeclaration(ColumnTypeFamily::FLOAT, 'REAL')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['a' => 1, 'b' => 2]], 'columns' => [], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['a' => 1], ['a' => 2, 'b' => 3]], 'columns' => [], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('"a"', $result);
        self::assertStringContainsString('"b"', $result);
    }
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [], 'columns' => [], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['data' => [1, 2, 3]]], 'columns' => ['data'], 'columnTypes' => []]];
    ...
    public function testObjectWithToString(): void
    {
        ...
        $tables = ['users' => ['rows' => [['val' => $obj]], 'columns' => ['val'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'Alice'], ['id' => 2, 'name' => 'Bob'], ['id' => 3, 'name' => 'Charlie']], 'columns' => ['id', 'name'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('UNION ALL', $result);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []], 'orders' => ['rows' => [['id' => 10]], 'columns' => ['id'], 'columnTypes' => []]];
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [], 'columns' => ['id'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => null]], 'columns' => ['id'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['data' => [1, 2]]], 'columns' => ['data'], 'columnTypes' => ['data' => new ColumnDeclaration(ColumnTypeFamily::TEXT, 'TEXT')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1]], 'columns' => [], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('"id"', $result);
    }
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [], 'columns' => ['id', 'name'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['orders' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []]];
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []], 'orders' => ['rows' => [['oid' => 10]], 'columns' => ['oid'], 'columnTypes' => []]];
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [], 'columns' => [], 'columnTypes' => []], 'orders' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users, orders', $tables);
        self::assertStringContainsString('"orders"', $result);
    }
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['x' => 1, 'y' => 2], ['x' => 3, 'y' => 4]], 'columns' => [], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('UNION ALL', $result);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['name' => 'Alice']], 'columns' => ['name'], 'columnTypes' => ['name' => new ColumnDeclaration(ColumnTypeFamily::TEXT, 'TEXT')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString("CAST('Alice' AS TEXT) AS \"name\"", $result);
    }
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['a' => null]], 'columns' => ['a'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['flag' => true]], 'columns' => ['flag'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['flag' => false]], 'columns' => ['flag'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['price' => 3.14]], 'columns' => ['price'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['age' => 25]], 'columns' => ['age'], 'columnTypes' => ['age' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('CAST(', $result);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['val' => 1.5]], 'columns' => ['val'], 'columnTypes' => ['val' => new ColumnDeclaration(ColumnTypeFamily::FLOAT, 'FLOAT')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('CAST(', $result);
    }
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []], 'orders' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('"users"', $result);
        self::assertStringNotContainsString('"orders"', $result);
    }
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['name' => "O'Brien"]], 'columns' => ['name'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
    ...
    public function testFormatValueWithTypeObjectUsesSerialized(): void
    {
        $transformer = new SelectTransformer();
        $obj = new stdClass();
        $tables = ['users' => ['rows' => [['val' => $obj]], 'columns' => ['val'], 'columnTypes' => ['val' => new ColumnDeclaration(ColumnTypeFamily::TEXT, 'TEXT')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('CAST(', $result);
    }
    ...
    public function testTransformWithCustomCastRendererUsesIt(): void
    {
    ...
    public function testTransformWithCustomQuoterUsesIt(): void
    {
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['notinquery' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => []], 'users' => ['rows' => [['id' => 2]], 'columns' => ['id'], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('"users" AS (', $result);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['x' => 10]], 'columns' => [], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('"x"', $result);
    ...
    {
        $transformer = new SelectTransformer();
        $tables = ['users' => ['rows' => [['id' => 1, 'name' => 'A'], ['id' => 2, 'name' => 'B']], 'columns' => [], 'columnTypes' => []]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
        self::assertStringContainsString('UNION ALL', $result);
        self::assertStringContainsString('"id"', $result);
    ...
    public function testCustomCastRendererIsUsedForRows(): void
    {
        ...
        $transformer = new SelectTransformer($castRenderer, null);
        $tables = ['users' => ['rows' => [['id' => 1]], 'columns' => ['id'], 'columnTypes' => ['id' => new ColumnDeclaration(ColumnTypeFamily::INTEGER, 'INTEGER')]]];
        $result = $transformer->transform('SELECT * FROM users', $tables);
